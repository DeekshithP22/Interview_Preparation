...
class WebSearchAgent:
    ...
    #     # Check if it's HCP search or workplace search based on response structure
    #     if "main_search" in clean_response["search_results"]:
    #         # HCP search deduplication (has main_search, linkedin_search, workplace_search)
            
    #         # Process main search results
    #         deduplicated_main = []
    #         for result in clean_response["search_results"]["main_search"]["results"]:
    #             url = result.get("url", "")
    #             if url not in seen_urls:
    #                 seen_urls.add(url)
    #                 deduplicated_main.append(result)
            
    #         # Process LinkedIn search results
    #         deduplicated_linkedin = []
    #         for result in clean_response["search_results"]["linkedin_search"]["results"]:
    #             url = result.get("url", "")
    #             if url not in seen_urls:
    #                 seen_urls.add(url)
    #                 deduplicated_linkedin.append(result)
            
    #         # Process workplace search results
    #         deduplicated_workplace = []
    #         for result in clean_response["search_results"]["workplace_search"]["results"]:
    #             url = result.get("url", "")
    #             if url not in seen_urls:
    #                 seen_urls.add(url)
    ...
    #         # Update the response
    #         clean_response["search_results"]["main_search"]["results"] = deduplicated_main
    ...
    #     else:
    #         # Workplace search deduplication (has workplace_validation)
    #         deduplicated_workplace_validation = []
    #         for result in clean_response["search_results"]["workplace_validation"]["results"]:
    #             url = result.get("url", "")
    #             if url not in seen_urls:
    #                 seen_urls.add(url)
    ...
    async def search_hcp(self, doctor_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # #Remove Duplicates across all search results
            # logger.info("Removing duplicates across all search results...")   
            # clean_response = self._deduplicate_results(clean_response)
    ...
    async def search_workplace(self, workplace_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        #    # Remove Duplicates across all search results
        #     logger.info("Removing duplicates across workplace search results...")
        #     clean_response = self._deduplicate_results(clean_response)
