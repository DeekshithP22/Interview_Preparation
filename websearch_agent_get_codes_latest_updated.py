...
class AsyncTavilyWrapper:
    """Robust async wrapper for Tavily search operations with error handling"""
    ...
    async def search_async(self, **kwargs):
        """Robust async wrapper with retries, timeouts, and rate limiting"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                search_task = loop.run_in_executor(
                    None, 
                    lambda: self.tavily_client.search(**kwargs)
                )
                
                # 30-second timeout
                result = await asyncio.wait_for(search_task, timeout=30.0)
                ...
                logger.warning(f"Tavily request timeout (attempt {attempt + 1}/{max_retries})")
                if attempt == max_retries - 1:
                    return {"results": [], "answer": ""}
                await asyncio.sleep(base_delay * (2 ** attempt))
                
            except Exception as e:
                logger.warning(f"Tavily request failed (attempt {attempt + 1}/{max_retries}): {str(e)}")
                if attempt == max_retries - 1:
                    # Return empty but valid structure on final failure
                    return {"results": [], "answer": ""}
                await asyncio.sleep(base_delay * (2 ** attempt))
...
class AsyncLLMWrapper:
    """Robust async wrapper for LLM operations with error handling"""
    
    def __init__(self, llm):
        self.llm = llm
        self._last_request_time = 0
        self._min_interval = 0.2  # Minimum 200ms between LLM requests
        
    async def invoke_async(self, prompt):
        """Robust async LLM invoke with retries and rate limiting"""
        ...
        max_retries = 2  # Fewer retries for LLM
        base_delay = 2.0
        ...
        for attempt in range(max_retries):
            try:
                ...
                loop = asyncio.get_event_loop()
                llm_task = loop.run_in_executor(None, lambda: self.llm.invoke(prompt))
                
                # 60-second timeout for LLM
                result = await asyncio.wait_for(llm_task, timeout=60.0)
                return result
                
            except asyncio.TimeoutError:
                logger.warning(f"LLM request timeout (attempt {attempt + 1}/{max_retries})")
                if attempt == max_retries - 1:
                    return "LLM analysis failed due to timeout"
        ...
        return "LLM analysis failed after retries"
...
class TavilyPayloadBuilder:
    ...
    def build_workplace_validation_payload(self, workplace_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "include_answer": True,
            "country": config.get("country", ""),
            "exclude_domains": config.get("exclude_domains", []) + [".com/reviews"],
            # "include_raw_content": True,
            # "time_range": "year",
...
class ResultValidator:
    ...
    def validate_url(self, result: Dict[str, Any], search_input: Dict[str, Any]) -> ValidationResult:
        """Flexible weighted validation with 0.3 threshold for maximum coverage"""
        url = result.get("url", "")
        title = result.get("title", "")
        content = result.get("content", "")
        ...
        if entity_type == "ent_activity" or (first_name and last_name):
            ...
            # Lower threshold for maximum coverage while filtering garbage
            score_threshold = 0.3
            is_valid = final_score >= score_threshold
            
            # Build detailed validation reasons
            validation_reasons = []
            ...
            # Add context about acceptance/rejection reasoning
            if is_valid:
                if exact_name_match:
            ...
            # Use final score as confidence
            confidence_score = final_score if is_valid else 0.0
            
            # Check workplace context (informational)
            workplace_match = self.validate_exact_workplace_match(full_content, workplace)
            if workplace_match:
                validation_reasons.append(" Bonus: Workplace context found")
